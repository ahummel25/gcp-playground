...
```mermaid
graph TD
    A[⏰ Cloud Scheduler<br/>Daily @ UTC] --> B[🚀 EmptyCommit Handler]
    
    B --> C[✅ Validate GITHUB_REPO]
    B --> D[🔐 Get GitHub Token<br/>Secret Manager]
    
    C --> E[📝 Parse Owner/Repo]
    D --> E
    
    E --> F[🔍 GraphQL Query<br/>Get Repository Info]
    F --> G[📊 Extract Repo ID & HEAD SHA]
    G --> H[✨ GraphQL Mutation<br/>Create Verified Commit]
    H --> I[📦 Return Commit SHA]
    I --> J[📤 HTTP Response]
...
4. Reference the function in `environments/prod/main.tf` using the `cloud-function` module
